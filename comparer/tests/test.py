    ...
    def generate_test(params):
      def test(self):
        comparison = ImageComparison(
            params['baseline'], params['comparee'], max_rms_error=params['max_rms_error']
        )
...
class DiffTest(unittest.TestCase):
  def test_it_should_render_a_diff_of_the_two_images(self):
    diff = ImageDiff(
        ...
        _get_file_from_test_dir("comparison_very_different_001.png"), TEST_DATA_DIR
    )

    path_to_diff = diff.create()

    comparison = ImageComparison(
        path_to_diff, _get_file_from_test_dir("diff_test_different.png"), max_rms_error=0
    )
  ...
  def test_the_diff_should_be_blank_if_the_images_are_the_same(self):
    diff = ImageDiff(
        _get_file_from_test_dir("converter_test_001.png"),
        _get_file_from_test_dir("converter_test_001.png"), TEST_DATA_DIR
    )

    path_to_diff = diff.create()

    comparison = ImageComparison(
        path_to_diff, _get_file_from_test_dir("diff_test_same.png"), max_rms_error=0
    )
